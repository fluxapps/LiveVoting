...
if (! class_exists('QRcode', false)) {
	...
	class QRcode {
			...
			for ($i = 0; $i < $length; ++ $i) {
				...
				if ($i & 1) {
					if (($i >= 3) AND ($i < ($length - 2)) AND ($this->runLength[$i] % 3 == 0)) {
						...
						if (($this->runLength[$i - 2] == $fact) AND ($this->runLength[$i - 1] == $fact) AND ($this->runLength[$i + 1] == $fact)
		...
		protected function encodeModeKanji($inputitem, $version) {
			...
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(QR_MODE_KJ, $version), (int)($inputitem['size']
				/ 2));
		...
		protected function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
			foreach ($this->rsitems as $rs) {
				if (($rs['pad'] != $pad) OR ($rs['nroots'] != $nroots) OR ($rs['mm'] != $symsize) OR ($rs['gfpoly'] != $gfpoly) OR ($rs['fcr']
						!= $fcr) OR ($rs['prim'] != $prim)
				) {
					continue;
				}
