...
class ilLiveVotingVote {
	...
	function doCreate() {
		...
		$ilDB->manipulate("INSERT INTO rep_robj_xlvo_vote " . "(id, option_id, usr_id, usr_session) VALUES ("
			. $ilDB->quote($this->getId(), "integer") . "," . $ilDB->quote($this->getOptionId(), "integer") . ","
			. $ilDB->quote($this->getUsrId(), "integer") . "," . $ilDB->quote($this->getUsrSession(), "text") . ")");
	...
	function doRead() {
		...
		$set = $ilDB->query("SELECT * FROM rep_robj_xlvo_vote " . " WHERE id = " . $ilDB->quote($this->getId(), "integer"));
	...
	function doDelete() {
		...
		$ilDB->manipulate("DELETE FROM rep_robj_xlvo_vote WHERE " . " id = " . $ilDB->quote($this->getId(), "integer"));
	}
