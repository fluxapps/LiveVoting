...
class ilLiveVotingContentGUI {
	...
	public function getHTML() {
		...
		if (get_class($this->local_ctrl) == 'ilCtrl' OR get_class($this->local_ctrl) == 'ilCtrl2') {
			...
			if (ilLiveVotingConfigGUI::_getValue("aspsms") AND
				ilLiveVotingConfigGUI::_getValue("aspsms_number") AND $this->live_voting->getAnonym()
			) {
				$tpl->setCurrentBlock("sms");
				$tpl->setVariable("SMS", $pl->txt("send_sms") . " <b>" . $this->live_voting->getPin()
					. " <span class='rotatePin'>X</span></b> " . $pl->txt("to_nr") . " <b>"
					. ilLiveVotingConfigGUI::_getValue("aspsms_number") . "</b>");
				$tpl->parseCurrentBlock();
			}
			if (ilLiveVotingConfigGUI::_getValue("sragsms") AND
				ilLiveVotingConfigGUI::_getValue("sragsms_number") AND $this->live_voting->getAnonym()
			) {
				$tpl->setCurrentBlock("sms");
				...
				$tpl->setVariable("SMS",
					$pl->txt("send_sms") . " <b>" . ilLiveVotingConfigGUI::_getValue("sragsms_keyword") . " "
					. $this->live_voting->getPin() . " <span class='rotatePin'>X</span></b> " . $pl->txt("to_nr")
					. " <b>" . ilLiveVotingConfigGUI::_getValue("sragsms_number") . "</b>" . $costs);
				$tpl->parseCurrentBlock();
			}
		...
		if ($this->live_voting_gui->hasPermission("write") AND
			$br->getAgent() != "IE" AND ilLiveVotingConfigGUI::_getValue("allow_fullscreen")
		) {
		...
		for ($i = 0; $i < count($options); $i ++) {
			...
			if( $this->live_voting->getColorful() )
				...
				$tpl->setVariable("CHOICE_COLOR", "0"); // use only one color
