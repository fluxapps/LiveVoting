...
class ctrlmmMultiLineInputGUI extends ilFormPropertyGUI {
	...
	public function addInput($input, $options = array()) {
	...
	public function render($iterator_id = 0, $clean_render = false)
	{
		...
		foreach($inputs as $key => $input)
		{
			...
			if(!method_exists($input, 'render')) {
				throw new ilException("Method ".get_class($input)."::render() does not exists! You cannot use this input-type in ilMultiLineInputGUI");
			}
	...
	public function insert(&$a_tpl) {
		...
		if($this->getMulti()) {
			...
			$output .= '<script type="text/javascript">$("#'.$this->getFieldId().'").multi_line_input('.json_encode($this->input_options).')</script>';
		}
	...
	public function getMultiIconsHTML($a_sortable = false) {
			...
			if(file_exists(ilUtil::getImagePath('edit_add.png'))) {
				...
				$html .= '<a href="#" style="display: inline-block;" class="remove_button"><img src="'.ilUtil::getImagePath('edit_remove.png').'" /></a>';
			} else {
